...
# Iterate over params in randomization_params_names to build configurations
for param_name in randomization_params_names:
    ...
    if randomization_params_eval[f"randomize_{param_name}"]:
        if "stiffness" in param_name or "damping" in param_name:
            ...
            # Build all values for this parameter
            ...
            for axis in directions:
                ...
                # Build all values for this parameter and axis
                current_value = min_val
...
# Run evaluations efficiently
print("Running evaluations for all configurations...")
time_all.append(timeit.default_timer())  # Compilation time marker
...
for config_idx, config in enumerate(eval_configs):
    ...
    run_evaluation_loop(env_state, n_steps, train_state, rng, prosthesis_metrics_handler,
                       param_name=param_name, param_value=param_value, direction=direction,
                       subfolder_name=subfolder_name, dt_str_init=dt_str_init)
...
print(f"Setup/compilation time: {time_all[1] - time_all[0]:.2f} seconds")
print(f"Execution time: {time_all[-1] - time_all[1]:.2f} seconds")
print(f"Average time per configuration: {(time_all[-1] - time_all[1]) / len(eval_configs):.2f} seconds")
print(f"{'='*80}")
