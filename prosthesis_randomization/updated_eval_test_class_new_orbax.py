...
randomization_params_eval = {
    ...
    "randomize_prosthesis_body_orientation": True, #False,
...
parser.add_argument('--ckpt_path', type=str, required=True, help='Path to the agent pkl file')
...
agent_conf= SavePPOJax.load_agent_conf(conf_path)
config = agent_conf.config
# agent_conf, agent_state = PPOJax.load_agent(path)
checkpoint_number = ckpt_path.split('_')[-1]
config = agent_conf.config
...
def run_evaluation_loop(env_state, n_steps, train_state, rng,prosthesis_metrics_handler, param_name=None, param_value=None, direction = None):
    ...
    checkpoint_number = ckpt_path.split("/")[-1]
    full_path = Path(ckpt_path)
    path =  str(full_path.parent.parent) + "/" #full_path.parent.parent +'/'
    if "param_name":
        joint_name = list(randomization_params_eval[f"{param_name}_range"].keys())[0]
        output_path = os.path.join(os.path.dirname(path), f"{dt_str}_eval_{checkpoint_number}_{n_steps}steps_{joint_name}_{direction}_{int(param_value*1000)}{param_name}.pkl")
    else:   
