...
def experiment(config: DictConfig):
    try: 
        ...
        # Add the results directory to the config for checkpointing
        # This will be accessible within _train_fn_continue via agent_conf.config.experiment.results_dir
        # with OmegaConf.open_dict(config.experiment):
        #     config.experiment.results_dir = result_dir
        ...
        # Extract final step or counter
        train_step = agent_state.train_state.step
        counter = ((train_step + 1) // config.experiment.num_minibatches) // config.experiment.update_epochs

        # Get timestep (first seed if multi-seed)
        final_timestep = int(jax.device_get(training_metrics[-1].max_timestep[0]))

        if config.checkpoint_interval > 0 and counter % config.checkpoint_interval == 0:
            save_path = SavePPOJax.save_agent(config.experiment.result_dir, agent_conf, agent_state, current_timestep=final_timestep)
            run.config.update({"agent_save_path": save_path})
