...
def experiment(config: DictConfig):
    try: 
        ...
        agent_conf, agent_state = SavePPOJax.load_agent(checkpoint_path)
        ...
        # agent_state.train_state.params['FullyConnectedNet_1'] = network_params['params']['FullyConnectedNet_1']
        ...
        # Build a per-seed training function
        def per_seed_train_fn(rng, agent_state):
            return SavePPOJax.build_train_fn_continue(env, agent_conf, agent_state=agent_state, mh=mh)(rng)

        # Vectorize and JIT the training function
        train_fn = jax.jit(jax.vmap(per_seed_train_fn))

        # Prepare batched RNGs and agent states
        skip_seed=0
        # config.experiment.n_seeds = 2
        # rngs = jnp.stack([jax.random.PRNGKey(i) for i in range(skip_seed,config.experiment.n_seeds + 1)])
        # agent_states = agent_state.train_state
        # # Run training for all seeds
        # out = train_fn(rngs, agent_states)


        rngs = [jax.random.PRNGKey(i) for i in range(skip_seed,config.experiment.n_seeds + 1)]
        rng, _rng = rngs[0], jnp.squeeze(jnp.vstack(rngs[1:]))
        agent_states = agent_state.train_state
        # Run training for all seeds
        out = train_fn(_rng, agent_states)
