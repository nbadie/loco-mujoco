...
class Mjx(Mujoco):
    ...
    def __init__(self, **kwargs):
        ...
        # Socket_ty slack 
        high_stiffness = self.socket_joint_stiffnesses['socket_ty']
        low_stiffness = self.socket_joint_stiffnesses['socket_ty']/10
        a = 0.02
        H = 0.02 
    ...
    def mjx_step(self, state: MjxState, action: jax.Array) -> MjxState:
        ...
        # Adapt Qfrc_applied to apply slack forces in the socket_ty joint
        def _adapt_qfrc_applied(_data): 

            for side in self.prosthesis_side: 
                joint_id = mujoco.mj_name2id(self._model, mujoco.mjtObj.mjOBJ_JOINT, f"socket_ty{side}")
                pistoning_qpos_adr = self._model.jnt_qposadr[joint_id]
        ...
        def _inner_loop(idx, _runner_state):
            ...
            # Apply socket_ty slack forces
            _data = jax.lax.cond(self.socket_ty_slack, _adapt_qfrc_applied, lambda x: x, _data)
    ...
    def mjx_render(self, state,
        ...
        """
        model = self._model
        should_visualize_randomization = (
            hasattr(self._domain_randomizer, 'visualize_randomization')
            and self._domain_randomizer.visualize_randomization
            and hasattr(self._domain_randomizer, '_update_mjM_from_domain_randomization')
        )
        ...
        per_env_model_update_fn = None
        if should_visualize_randomization:
            per_env_model_update_fn = lambda env_idx: self._domain_randomizer._update_mjM_from_domain_randomization(
                self._viewer._model,
                state.additional_carry,
                env_idx=env_idx,
            )

        return self._viewer.parallel_render(
            state,
            record,
            per_env_model_update_fn=per_env_model_update_fn,
        )
