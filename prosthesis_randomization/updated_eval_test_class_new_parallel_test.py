...
param_name = "prosthesis_joint_stiffness"
min_val = 1000
max_val = 1300
increment = 100
joint_name = 'ankle_angle'
if "prosthesis_side" in config.experiment.env_params:
    prosthesis_side = config.experiment.env_params["prosthesis_side"]
test_values = jnp.arange(min_val, max_val + increment, increment)  
num_variants = len(test_values)
...
# Get joint index for the joint to be modified
joint_name = f"{joint_name}_{prosthesis_side[0]}"  # e.g., 'ankle_angle_l' for left side
joint_idx = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_JOINT, joint_name)


def get_batched_model(base_model, values, jnt_idx):
    # This function creates a version of the model with a specific stiffness
    new_stiffness = base_model.jnt_stiffness.at[jnt_idx].set(values)
    return base_model.replace(jnt_stiffness=new_stiffness)

# Vectorize the model creation
batched_models = jax.vmap(get_batched_model, in_axes=(None, 0, None))(env.sys, test_values, joint_idx)
env.sys = batched_models
...
n_steps = 200
n_envs = 1 #num_variants # <--- Make sure this matches your training batch size
rng = jax.random.key(0)
train_state_seed = 0  # Take first seed 

keys = jax.random.split(rng, num_variants + 1)
...
# Reset the environment for all variants at once 
env_state = jit_reset(env_keys) #jax.vmap(env.mjx_reset)(env_keys)

# def update_stiffness(state, val): 
#     new_stiffness = state.model.jnt_stiffness.at[joint_idx].set(val)
#     return state.replace(model.tree_replace({'jnt_stiffness': new_stiffness}))
#         #model=state.model.replace(jnt_stiffness=new_stiffness))

#
# env_state = jit_reset(env_keys, batched_models)
# env_state = jit_step(env_state, action, batched_models)


# env_state = jax.vmap(update_stiffness)(env_state, test_values)
print("Updated joint stiffness for all variants. ", env.sys.jnt_stiffness[...,joint_idx])
...
def run_evaluation_loop(env_state, n_steps, train_state, rng,prosthesis_metrics_handler, param_name=None): #, param_value=None, direction=None):
    ...
    for i in range(n_steps):
        ...
        env_state = jit_step(env_state, action) #, batched_models)  #env.step(env_state, action)
        obs = env_state.observation
    ...
    for n_conf in range(num_variants):
        ...
        if "param_name":
            ...
            joint_name = param_name
            output_path = os.path.join(os.path.dirname(path), f"{dt_str}_eval_{n_steps}steps_{joint_name}.pkl") #_{direction}_{int(param_value*10)}{param_name}.pkl")
...
run_evaluation_loop(env_state, n_steps, train_state, rng, prosthesis_metrics_handler,param_name=param_name) #min_val + inc)
