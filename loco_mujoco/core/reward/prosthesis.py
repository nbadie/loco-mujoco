...
@struct.dataclass
class ProsthesisRewardState:
    """
    State of ProsthesisReward.
    """
    last_qvel: Union[np.ndarray, jnp.ndarray]
    last_action: Union[np.ndarray, jnp.ndarray]

class ProsthesisReward(MimicReward):
    """
    ...
    def __init__(self, env: Any,
        ...
        self._lateral_pos_range_coeff = kwargs.get("lateral_pos_range_coeff", 0.0)
        self._lateral_pos_range = kwargs.get("lateral_pos_range", 0.5)
    ...
    def init_state(self, env: Any,
        ...
        """
        return ProsthesisRewardState(last_qvel=data.qvel, last_action=backend.zeros(env.info.action_space.shape[0]))
    ...
    def __call__(self,
        ...
        if self._action_coeff > 0.0:
            ...
            action_reward = -action_norm 
        else: 
            action_reward = 0.0

        # lateral position reward for the free joint
        if self._lateral_pos_range_coeff>0.0:
            lateral_free_pos = data.qpos[1]
            lateral_pos_reward = jnp.where(jnp.logical_or(lateral_free_pos < -self._lateral_pos_range, lateral_free_pos > self._lateral_pos_range), self._lateral_pos_range_coeff, 0.0)
        else: 
