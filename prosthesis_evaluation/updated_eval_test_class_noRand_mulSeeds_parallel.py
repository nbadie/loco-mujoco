...
n_steps = 700 #1000 #400 #2 #400 #400 #1000 #10 #1000 #00 #1000
n_envs = 1 #1  # <--- Make sure this matches your training batch size

# seed = 0
seeds = config.experiment.n_seeds #10 #4 #5 #10
...
def run_eval_for_seed(seed, train_state):
...
train_states = agent_state.train_state if config.experiment.n_seeds > 1 else [agent_state.train_state]

# # Convert train_states to a JAX array if needed
# train_states = jax.tree.map(lambda x: jnp.array(x), train_states)
# results = jax.vmap(run_eval_for_seed)(seed_values, train_states)

train_states = np.array([train_states] * len(seed_values))
results = jax.vmap(lambda seed: run_eval_for_seed(seed, train_states))(seed_values)
