...
def evaluation_step_jit(carry, step_i):
    """Non-JIT evaluation step to match your working original code"""
...
def extract_metrics_from_step_data(step_data_collected):
    """Extract all metrics from step data outside of JAX compilation"""
    # step_data_collected is a tuple: (env_states, actions, step_indices)
    env_states, actions, step_indices = step_data_collected
...
def run_single_configuration_evaluation(param_config, env_state, train_state, rng):
    ...
    # Run the simulation loop (not JIT-compiled to match your original)
    ...
    # Apply domain randomization rendering for the final state
...
def run_improved_evaluation():
    ...
    for i, param_config in enumerate(all_configs):
        ...
        # Apply domain randomization to the MJX environment state
        env.mjx_render_domain_randomization(env_state, record=True)
