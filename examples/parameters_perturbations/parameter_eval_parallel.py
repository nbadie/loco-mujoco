...
def _split_step_rngs(step_rngs):
    step_rngs_split = jax.vmap(lambda r: jax.random.split(r, 2))(step_rngs)
    return step_rngs_split[:, 0], step_rngs_split[:, 1]
...
def _generate_output_path(save_dir, param_info, seed_idx):
    ...
    return os.path.join(out_folder, f"seed{seed_idx}.pkl")
...
def main():
    ...
    config.experiment.env_params["headless"] = False #True #False
    ...
    def run_evaluation_parallel(
        ...
        for t in range(n_steps):
            if n_seeds > 1:
                step_rngs, action_rngs = _split_step_rngs(step_rngs)
            else:    
                rng, action_rngs = jax.random.split(rng)


            # Get actions for all seeds
            obs = env_states.observation
            actions, current_train_state = sample_actions(current_train_state, obs, action_rngs)
            if n_seeds == 1:
            ...
            env.mjx_render(env_states, record=True)
    ...
    def reset_and_eval(param_name, current_value, axis_or_joint, body_or_joint_name):
        ...
        env_states = jit_reset(rng_seeds)
