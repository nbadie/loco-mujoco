...
parser.add_argument('--path1', type=str, required=True, help='Path to the first agent pkl file')
parser.add_argument('--path2', type=str, required=True, help='Path to the second agent pkl file')
parser.add_argument('--path3', type=str, required=True, help='Path to the third agent pkl file')
args = parser.parse_args()

# --- Load Agent ---
# path = [args.path1, args.path2, args.path3]
path1 = args.path1
path2 = args.path2
path3 = args.path3

agent_conf_1, agent_state_1 = PPOJax.load_agent(path1)
agent_conf_2, agent_state_2 = PPOJax.load_agent(path2)
agent_conf_3, agent_state_3 = PPOJax.load_agent(path3)

dict_1 = to_state_dict(agent_state_1.train_state)
dict_2 = to_state_dict(agent_state_2.train_state)
dict_3 = to_state_dict(agent_state_3.train_state)
...
merged_dict = jax.tree.map(stack_three_seeds, dict_1, dict_2, dict_3)

# 6. Reconstruct the TrainState object
merged_train_state = from_state_dict(agent_state_1.train_state, merged_dict)

config = agent_conf_1.config

n_seeds = config.experiment.n_seeds  # Get total seeds from config

skip_0_alignment = True #False
skip_3deg_alignment = True 

# subfolder_name = 'multiInput_eval_pylon_socket_pos_x_talus_ori_z-3deg' #
subfolder_name = 'multiInput_eval_pylon_socket_ori_z_-5to5_seed8_11'
...
randomization_params_eval = {
    ...
    # "prosthesis_body_position_range": {'pylon_socket': {'z': [0.005,0.010]}},
    "prosthesis_body_position_range": {'pylon_socket': {'x': [-0.010,0.010]}},
    # "prosthesis_body_position_range": {'pylon_socket': {'z': [-0.010,-0.005]}},
    "randomize_prosthesis_body_orientation": True, #True, #True,
    # "prosthesis_body_orientation_range": {'pylon_socket': {'z': [-np.deg2rad(6), np.deg2rad(6)]}} 
    # "prosthesis_body_orientation_range": {'pylon_socket': {'z': [-np.deg2rad(6), np.deg2rad(6)]}, 'talus': {'z': [-np.deg2rad(3), -np.deg2rad(3)]}} 
    # "prosthesis_body_orientation_range": {'talus': {'z': [np.deg2rad(3), np.deg2rad(3)]}} 
    "prosthesis_body_orientation_range": {'pylon_socket': {'z': [-np.deg2rad(5), np.deg2rad(5)]}} 
    # "prosthesis_body_orientation_range": {'pylon_socket': {'x': [-np.deg2rad(6), np.deg2rad(6)]}} 
}
...
randomization_increments = {
    ...
    # "prosthesis_body_orientation": np.deg2rad(6)*2, #
    # "prosthesis_body_orientation": np.deg2rad(6)/2, #
    "prosthesis_body_orientation": np.deg2rad(5)/5, #0.35 #0.001 #0.2
}

talus_ori_unequal_0 = False #True #False
...
rng_seeds = jnp.array([jax.random.PRNGKey(i) for i in range(8,n_seeds+8)])
...
def sample_actions_uncompiled(ts, obs, _rng):
    y, updates = agent_conf_1.network.apply({'params': ts.params, 'run_stats': ts.run_stats},
...
def run_evaluation_parallel(n_steps,n_seeds, env_states,merged_train_state,rng, save_dir, param_info=None):#subfolder, dt_init, param_info=None):
    ...
    curr_train_states = merged_train_state #agent_state.train_state
...
save_dir = os.path.join(os.path.dirname(path1), f'{dt_init}_{subfolder_name}')
        ...
        if "stiffness" in param_name or "damping" in param_name:
            ...
            while current_value <= max_val:
                ...
                run_evaluation_parallel(n_steps=2000,
                                            ...
                                            merged_train_state=merged_train_state,
                                            rng=rng_seeds, #rng,
                                            save_dir=save_dir,
            ...
            for axis in directions:
                ...
                while current_value <= max_val + 1e-6:
                    ...
                    if skip_0_alignment and current_value == 0:
                        current_value += inc
                        continue
                    if skip_3deg_alignment and np.abs(current_value) == np.deg2rad(3):
                        current_value += inc
                        continue
