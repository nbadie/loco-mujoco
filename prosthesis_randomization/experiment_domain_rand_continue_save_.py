...
def experiment(config: DictConfig):
    try: 
        ...
        print('NUM_STEPS', int(config.experiment.total_timesteps // config.experiment.num_steps // config.experiment.num_envs))
        ...
        train_fn = SavePPOJax.build_train_fn_continue(env, agent_conf, agent_state=agent_state, mh=mh) #PPOJax.build_train_fn(env, agent_conf, mh=mh)
        ...
        intermediate_train_states = out["intermediate_train_states"]
        save_flags = out["save_flags"]

        # Example for vmap: iterate over each seed's results
        num_updates = int(config.experiment.total_timesteps // config.experiment.num_steps // config.experiment.num_envs) #config.experiment.num_updates
        num_seeds = config.experiment.n_seeds if config.experiment.n_seeds > 1 else 1
        print('NUM_UPDATES', num_updates)
        for seed_idx in range(num_seeds):
            print(f"Processing checkpoints for seed {seed_idx}...")

            # JAX's tree_map for extracting the current seed's train_states
            current_seed_intermediate_train_states = jax.tree_map(
                lambda x: x[..., seed_idx], intermediate_train_states
            ) if num_seeds > 1 else intermediate_train_states
            import time
            print(f"Starting checkpoint saving loop for seed {seed_idx} at {time.time()}...")
            for i in range(num_updates):
                if save_flags[i]:
                    print(f"  Attempting to save checkpoint for step {i} (global step {current_seed_intermediate_train_states.step[i]}) at {time.time()}...")

                    step_train_state = jax.tree_map(lambda x: x[i], current_seed_intermediate_train_states)
                    step_for_checkpoint = step_train_state.step
                    checkpoint_agent_state = SavePPOJax._agent_state(
                        train_state=step_train_state,
                        agent_conf=agent_conf
                    )

                    # Ensure result_dir is a Path object for easy joining
                    from pathlib import Path
                    result_dir_path = Path(result_dir) 

                    if num_seeds > 1:
                        checkpoint_filename = result_dir_path / f"PPOJax_seed_{seed_idx}_step_{step_for_checkpoint}.pkl"
                    else:
                        checkpoint_filename = result_dir_path / f"PPOJax_step_{step_for_checkpoint}.pkl"

                    SavePPOJax.save_agent(checkpoint_agent_state, str(checkpoint_filename))
                    print(f"  Successfully saved checkpoint to {checkpoint_filename} at {time.time()}")
                # else:
                #     print(f"  Skipping save for step {i} (flag is False)") # Optional: to see if flags are mostly False

            print(f"Finished checkpoint saving loop for seed {seed_idx} at {time.time()}")

        # Add prints for final save as well
        print(f"Starting final agent state save at {time.time()}...")
        if num_seeds > 1:
            for seed_idx in range(num_seeds):
                final_seed_agent_state = jax.tree_map(lambda x: x[seed_idx], final_agent_state)
                final_checkpoint_filename = result_dir_path / f"PPOJax_final_seed_{seed_idx}_step_{final_seed_agent_state.train_state.step}.pkl"
                SavePPOJax.save_agent(final_seed_agent_state, str(final_checkpoint_filename))
                print(f"Saved final agent state to {final_checkpoint_filename} at {time.time()}")
        else:
            final_checkpoint_filename = result_dir_path / f"PPOJax_final_step_{final_agent_state.train_state.step}.pkl"
            SavePPOJax.save_agent(final_agent_state, str(final_checkpoint_filename))
            print(f"Saved final agent state to {final_checkpoint_filename} at {time.time()}")
        print(f"Finished final agent state save at {time.time()}")

        # Add print before play_policy
        print(f"Starting play_policy for video recording at {time.time()}...")
        SavePPOJax.play_policy(env, agent_conf, final_agent_state if num_seeds==1 else final_seed_agent_state, deterministic=True, n_steps=200, n_envs=20, record=True, train_state_seed=0)
        print(f"Finished play_policy at {time.time()}")
